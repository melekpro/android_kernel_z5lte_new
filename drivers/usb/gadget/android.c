...
1. chuiguo.zeng@ragentek.com 2014.02.13 BUG_ID:DLEY-218
   Description: Show vendor information when connect to PC(Win7 printer device in control pannel).
************************************************************************/
#ifdef pr_fmt
#undef pr_fmt
#endif
...
extern int replace_character(char ** str, char *rchar, char *dchar );
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	//add BUG_ID:JWLJ-260 zengchuiguo 20140409 (start)
	char *prt_product = product_string;
	//add BUG_ID:JWLJ-260 zengchuiguo 20140409 (end)
	...
	//modify BUG_ID:JWLW-1205 daizhiyi 20131031 (start)
	...
	strncpy(manufacturer_string, CONFIG_RGK_USB_DRIVER_DISPLAY_MANUFACTURER, sizeof(manufacturer_string) - 1);
	if (strlen(manufacturer_string) == 0) {
		strncpy(manufacturer_string, MANUFACTURER_STRING, sizeof(manufacturer_string) - 1);
	}
#else
	strncpy(manufacturer_string, MANUFACTURER_STRING, sizeof(manufacturer_string) - 1);
	...
	strncpy(product_string, CONFIG_RGK_USB_DRIVER_DISPLAY_PRODUCT, sizeof(product_string) - 1);

	//add BUG_ID:JWLJ-260 zengchuiguo 20140409 (start)			
	replace_character(&prt_product,"^"," ");	
	//add BUG_ID:JWLJ-260 zengchuiguo 20140409 (end)
	...
	strncpy(product_string, PRODUCT_STRING, sizeof(product_string) - 1);
#endif
	//modify BUG_ID:JWLW-1205 daizhiyi 20131031 (end)
