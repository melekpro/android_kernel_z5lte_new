...
unsigned int charging_parameter_to_value(const unsigned int *parameter, const unsigned int array_size, const unsigned int val)
{
    ...
    battery_log(BAT_LOG_CRTI, "array_size = %d \r\n", array_size);
...
#if 0//lisong 2014-8-7 [NO BUGID][input_cs_vth should large than the current setting]start
...
#endif//lisong 2014-8-7 [NO BUGID][input_cs_vth should large than the current setting]end
...
static unsigned int charging_hw_init(void *data)
{
    ...
    pr_notice("[BATTERY:ncp1851] ChargerHwInit_ncp1851\n");

    ncp1851_status = ncp1851_get_chip_status();

    pmic_set_register_value(PMIC_RG_BC11_BB_CTRL,1);//upmu_set_rg_bc11_bb_ctrl(1);    //BC11_BB_CTRL    
    pmic_set_register_value(PMIC_RG_BC11_RST,1);//upmu_set_rg_bc11_rst(1);        //BC11_RST
...
static unsigned int charging_set_cv_voltage(void *data)
{
    ...
    if (batt_cust_data.high_battery_voltage_support)
        ...
        cv_value = BATTERY_VOLT_04_250000_V;
...
{
    //kick pmic wdt?
    pmic_set_register_value(PMIC_RG_CHRWDT_TD,0x0);//upmu_set_rg_chrwdt_td(0x0);           // CHRWDT_TD, 4s
    pmic_set_register_value(PMIC_RG_CHRWDT_INT_EN,0x1);//upmu_set_rg_chrwdt_int_en(1);         // CHRWDT_INT_EN
    pmic_set_register_value(PMIC_RG_CHRWDT_EN,0x1);//upmu_set_rg_chrwdt_en(1);             // CHRWDT_EN
    pmic_set_register_value(PMIC_RG_CHRWDT_WR,0x1);//upmu_set_rg_chrwdt_wr(1);             // CHRWDT_WR
