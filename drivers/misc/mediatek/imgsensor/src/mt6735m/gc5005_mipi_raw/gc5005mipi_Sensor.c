...
#define WB_BASE_ADDR 0x0f
#define CUSTOM_BASE_ADDR 0x16 //for custom
#define CUSTOM_MAX_SIZE 15//for custom
...
static kal_uint16 custom_addr[CUSTOM_MAX_SIZE];
static kal_uint16 custom_value[CUSTOM_MAX_SIZE];
static kal_uint8 custom_num=0;
...
void GC5005_MIPI_READ_OTP_INFO_GROUP1(void)
{
	otp_info.module_id = GC5005_MIPI_READ_OTP(0x01);
	otp_info.lens_id = GC5005_MIPI_READ_OTP(0x02);
	otp_info.vcm_driver_id = GC5005_MIPI_READ_OTP(0x03);
	otp_info.vcm_id = GC5005_MIPI_READ_OTP(0x04);	
	otp_info.year = GC5005_MIPI_READ_OTP(0x05); 
	otp_info.month = GC5005_MIPI_READ_OTP(0x06);
	otp_info.day = GC5005_MIPI_READ_OTP(0x07);
	LOG_INF("module_id=0x%x",otp_info.module_id);
	LOG_INF("lens_id=0x%x",otp_info.lens_id);
	LOG_INF("vcm_driver_id=0x%x",otp_info.vcm_driver_id);	
	LOG_INF("vcm_id=0x%x",otp_info.vcm_id);
	LOG_INF("data=%d-%d-%d",otp_info.year,otp_info.month,otp_info.day);
}
void GC5005_MIPI_READ_OTP_INFO_GROUP2(void)
{
	otp_info.module_id = GC5005_MIPI_READ_OTP(0x08);
	otp_info.lens_id = GC5005_MIPI_READ_OTP(0x09);
	otp_info.vcm_driver_id = GC5005_MIPI_READ_OTP(0x0a);
	otp_info.vcm_id = GC5005_MIPI_READ_OTP(0x0b);	
	otp_info.year = GC5005_MIPI_READ_OTP(0x0c); 
	otp_info.month = GC5005_MIPI_READ_OTP(0x0d);
	otp_info.day = GC5005_MIPI_READ_OTP(0x0e);
	LOG_INF("module_id=0x%x",otp_info.module_id);
	LOG_INF("lens_id=0x%x",otp_info.lens_id);
	LOG_INF("vcm_driver_id=0x%x",otp_info.vcm_driver_id); 
	LOG_INF("vcm_id=0x%x",otp_info.vcm_id);
...
void GC5005_MIPI_WRITE_SRAM_WB_GROUP1(void)
{
	kal_uint8 r_gain,g_gain,b_gain;
	r_gain = GC5005_MIPI_READ_OTP(0x0f) ;
	g_gain = GC5005_MIPI_READ_OTP(0x10) ;
	b_gain = GC5005_MIPI_READ_OTP(0x11) ;	
	write_cmos_sensor(0xfe,0x00);
...
void GC5005_MIPI_WRITE_SRAM_WB_GROUP2(void)
{
	kal_uint8 r_gain,g_gain,b_gain;
	r_gain = GC5005_MIPI_READ_OTP(0x12) ;
	g_gain = GC5005_MIPI_READ_OTP(0x13) ;
	b_gain = GC5005_MIPI_READ_OTP(0x14) ;	
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xb8,g_gain);
	write_cmos_sensor(0xb9,g_gain);
	write_cmos_sensor(0xba,r_gain);
...
void GC5005_MIPI_READ_SRAM_CUSTOM(void)
{
	kal_uint8 i=0;
	custom_num = GC5005_MIPI_READ_OTP(0x15) & 0x0f;
	LOG_INF("GC5005_MIPI_READ_SRAM_CUSTOM:custom_num = %d\n",custom_num);
	for(i=0;i<custom_num;i++)
	{
		custom_addr[i] = GC5005_MIPI_READ_OTP(CUSTOM_BASE_ADDR + i*2 ) ;
		custom_value[i] = GC5005_MIPI_READ_OTP(CUSTOM_BASE_ADDR + i*2 + 1 ) ;
	LOG_INF("GC5005_MIPI_READ_SRAM_CUSTOM:custom_addr[%d] = 0x%x,custom_value[%d] = 0x%x\n",i,custom_addr[i],i,custom_value[i]);		
	}
}
void GC5005_MIPI_WRITE_SRAM_CUSTOM(void)
{
	kal_uint8 i=0;
	write_cmos_sensor(0xfe,0x00);
	for(i=0;i<custom_num;i++)
	{
		write_cmos_sensor(custom_addr[i] ,custom_value[i]);
		LOG_INF("GC5005_MIPI_WRITE_SRAM_CUSTOM,{0x%x,0x%x}!!\n",custom_addr[i],custom_value[i]);		
	}
	write_cmos_sensor(0xfe,0x00);	
}
void GC5005_MIPI_OTP(void)
{
	...
	kal_uint8 check_info_flag=0;
	kal_uint8 check_wb_flag=0;	
	kal_uint8 check_custom_flag=0;
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xfe,0x00);
	write_cmos_sensor(0xfe,0x00);
	...
	GC5005_MIPI_select_page_OTP(OTP_SELECT_PAGE1);	
	check_info_flag = GC5005_MIPI_READ_OTP(0x00);
	switch(check_info_flag&0x30)
	{
	case 0x00:
		LOG_INF("GC5005_MIPI_OTP: OTP info group1 is Empty !!\n");
		break;
	case 0x10:
		GC5005_MIPI_READ_OTP_INFO_GROUP1();
		break;
	case 0x20:
		LOG_INF("GC5005_MIPI_OTP: OTP info group1 is Invalid !!\n");
		break;
	default :
		break;
	}	
	if((check_info_flag&0x30)!=0x10)
	{
		switch(check_info_flag&0xc0)
		{
		case 0x00:
			LOG_INF("GC5005_MIPI_OTP: OTP info group2 is Empty !!\n");
			break;
		case 0x40:
			GC5005_MIPI_READ_OTP_INFO_GROUP2();
			break;
		case 0x80:
			LOG_INF("GC5005_MIPI_OTP: OTP info group2 is Invalid !!\n");
			break;
		default :
			break;
		}		
	}
	check_wb_flag = GC5005_MIPI_READ_OTP(0x00);
	switch(check_wb_flag&0x03)
	{
	case 0x00:
		LOG_INF("GC5005_MIPI_OTP: OTP WB_Group1 is Empty !!\n");
		break;
	case 0x01:
		GC5005_MIPI_WRITE_SRAM_WB_GROUP1();
		break;
	case 0x02:
		LOG_INF("GC5005_MIPI_OTP: OTP WB_Group1 is Invalid !!\n");
		break;
	default :
		break;
	}	
	if((check_wb_flag&0x03)!=0x01)
	{
		switch(check_wb_flag&0x0c)
		{
		case 0x00:
			LOG_INF("GC5005_MIPI_OTP: OTP WB_Group2 is Empty !!\n");
			break;
		case 0x04:
			GC5005_MIPI_WRITE_SRAM_WB_GROUP2();
			break;
		case 0x08:
			LOG_INF("GC5005_MIPI_OTP: OTP WB_Group2 is Invalid !!\n");
			break;
		default :
			break;
		}		
	}	
	check_custom_flag = GC5005_MIPI_READ_OTP(0x15);
	switch(check_custom_flag&0xc0)
	{
	case 0x00:
		LOG_INF("GC5005_MIPI_OTP: SRAM_CUSTOM is Empty !!\n");
		break;
	case 0x40:
		GC5005_MIPI_READ_SRAM_CUSTOM();//GC5005_MIPI_REG_DIFF_CTL_GROUP();
		break;
	case 0x80:
		LOG_INF("GC5005_MIPI_OTP: SRAM_CUSTOM is Invalid !!\n");
		break;
	default :
		break;
	}
...
#define ANALOG_GAIN_2 96   // 1.469x
#define ANALOG_GAIN_3 138  // 2.165x
#define ANALOG_GAIN_4 198  // 3.105x
#define ANALOG_GAIN_5 282  // 3.632x
#define ANALOG_GAIN_6 395  // 4.750x
#define ANALOG_GAIN_7 548  // 8.569x
#define ANALOG_GAIN_8 757  // 11.69x
...
static void sensor_init(void)
{
	...
	/*Degrid*/
	//write_cmos_sensor(0x8a, 0x0a); //0b //[3:2]иигд??б┴ио
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("GC5005 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("GC5005 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
