...
#include <linux/seq_file.h>
static char wtk_camera_back[32]= "NoBackCamera";
static char wtk_camera_front[32] = "NoFrontCamera";

static int wtk_camera_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "BackCamera:%s,FrontCamera:%s\n", wtk_camera_back,wtk_camera_front);	
	return 0;
}
...
static const struct file_operations wtk_camera_info_proc_fops = {	
	.owner      = THIS_MODULE,
...
{
	struct proc_dir_entry *camera_info_entry = proc_create("wtk_cameraInfo", 0444, NULL, &wtk_camera_info_proc_fops);
	if (camera_info_entry == NULL)
...
#if 1//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]start
					if(DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[i]){ 
    					strncpy(wtk_camera_front, (char*)g_invokeSensorNameStr[i], sizeof(wtk_camera_front)-1);
    					PK_DBG("%scall front wtk_camera_front %s \n", __func__, wtk_camera_front);
					}else{
    					strncpy(wtk_camera_back, (char*)g_invokeSensorNameStr[i], sizeof(wtk_camera_back)-1);
    					PK_DBG("%s call back wtk_camera_back = %s \n", __func__, wtk_camera_back);
					}
#endif//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]end
