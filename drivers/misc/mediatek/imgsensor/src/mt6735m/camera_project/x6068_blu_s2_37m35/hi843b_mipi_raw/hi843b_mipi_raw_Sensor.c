...
#define Hi843B_OTP_FUNCTION    1
#if Hi843B_OTP_FUNCTION //Hi-843B OPT

//#define RG_Ratio_Typical (0x17B)
//#define BG_Ratio_Typical (0x13C)

void HI843B_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain);
kal_uint16 HI843B_Sensor_OTP_read(kal_uint16 otp_addr);
...
#if Hi843B_OTP_FUNCTION //Hi-843B OPT fuction begin
...
void Hi843B_Sensor_OTP_info(void)
{
    ...
    switch (info_flag)
  {
	...
	checksum = (ModuleHouseID + AFFF + Year + Month + Day + SensorID + LensID + VCMID + DriverID + FNoID ) % 256;
...
void HI843B_Sensor_update_wb_gain(kal_uint32 r_gain, kal_uint32 g_gain,kal_uint32 b_gain)
{
  ...
  temp = read_cmos_sensor(0x0a05) | 0x08;
   printk("%s, = 0x%x\n", __func__,temp);
    HI843B_OTP_write_cmos_sensor(0x6000, temp); //Digital Gain enable
	...
	switch (wb_flag)
	{
		case 0x01:  //Group1 valid
		   ...
		   // wb_golden_g_l = HI843B_Sensor_OTP_read(0x0c77);
			wbcheck = HI843B_Sensor_OTP_read(0x0c7d);
			
			for(i=0;i < 12;i++)
			{
				addr = 0x0c60 + i;
				LOG_INF("HI843B_Sensor: group 1 addr = 0x%x  0x%x\n ",i+0x0c60,HI843B_Sensor_OTP_read(addr));
			}
			break;
			
		case 0x02:  //Group2 valid
			wb_unit_rg_h = HI843B_Sensor_OTP_read(0x0c7e);
		    wb_unit_rg_l = HI843B_Sensor_OTP_read(0x0c7f);
		    wb_unit_bg_h = HI843B_Sensor_OTP_read(0x0c80);
		    wb_unit_bg_l = HI843B_Sensor_OTP_read(0x0c81);
		    wb_unit_gg_h = HI843B_Sensor_OTP_read(0x0c82);
		    wb_unit_gg_l = HI843B_Sensor_OTP_read(0x0c83);
		    wb_golden_rg_h = HI843B_Sensor_OTP_read(0x0c84);
		    wb_golden_rg_l = HI843B_Sensor_OTP_read(0x0c85);
		    wb_golden_bg_h = HI843B_Sensor_OTP_read(0x0c86);
		    wb_golden_bg_l = HI843B_Sensor_OTP_read(0x0c87);
		    wb_golden_gg_h = HI843B_Sensor_OTP_read(0x0c88);
		    wb_golden_gg_l = HI843B_Sensor_OTP_read(0x0c89);
		   ...
		   // wb_golden_g_l = HI843B_Sensor_OTP_read(0xBD51);
		    wbcheck = HI843B_Sensor_OTP_read(0x0c9b);
			for(i=0;i < 12;i++)
			{
				addr = 0x0c7e + i;
				LOG_INF("HI843B_Sensor: group 2 addr = 0x%x   0x%x\n ",i+0x0c7e,HI843B_Sensor_OTP_read(addr));
			}	
		    break;
		case 0x03:  //Group3 valid
		    wb_unit_rg_h = HI843B_Sensor_OTP_read(0x0c9c);
		    wb_unit_rg_l = HI843B_Sensor_OTP_read(0x0c9d);
		    wb_unit_bg_h = HI843B_Sensor_OTP_read(0x0c9e);
		    wb_unit_bg_l = HI843B_Sensor_OTP_read(0x0c9f);
		    wb_unit_gg_h = HI843B_Sensor_OTP_read(0x0ca0);
		    wb_unit_gg_l = HI843B_Sensor_OTP_read(0x0ca1);
		    wb_golden_rg_h = HI843B_Sensor_OTP_read(0x0ca2);
		    wb_golden_rg_l = HI843B_Sensor_OTP_read(0x0ca3);
		    wb_golden_bg_h = HI843B_Sensor_OTP_read(0x0ca4);
		    wb_golden_bg_l = HI843B_Sensor_OTP_read(0x0ca5);
		    wb_golden_gg_h = HI843B_Sensor_OTP_read(0x0ca6);
		    wb_golden_gg_l = HI843B_Sensor_OTP_read(0x0ca7);
		  ...
		  //  wb_golden_g_l = HI843B_Sensor_OTP_read(0xBD8A);
		    wbcheck = HI843B_Sensor_OTP_read(0x0cb9);
			for(i=0;i < 12;i++)
			{
				addr = 0x0c9c + i;
				LOG_INF("HI843B_Sensor: group 3 addr = 0x%x   0x%x\n ",i+0x0c9c,HI843B_Sensor_OTP_read(addr));
			}	
		    break;
		default:
...
//  wb_golden_bg_l = 0x09;//
 ...
 //     + wb_golden_r_h + wb_golden_r_l + wb_golden_b_h + wb_golden_b_l + wb_golden_g_h + wb_golden_g_l ) % 0xFF + 1;
	...
	if (checksum == wbcheck)
	{
		...
		printk("wangk_HI843B_Sensor: WB checksum PASS\n ");
    }
	else
    {
	    LOG_INF("HI843B_Sensor: WB checksum Fail\n ");
	    printk("wangk_HI843B_Sensor: WB checksum Fail\n ");
    }
    ...
    printk("%s,Read from module: r_gain = 0x%x, g_gain = 0x%x,b_gain = 0x%x\n",__func__, r_gain,g_gain,b_gain);

	rg_golden_value = ((wb_golden_rg_h << 8)|wb_golden_rg_l);  //0x0278
	bg_golden_value = ((wb_golden_bg_h << 8)|wb_golden_bg_l);  //0x0252
...
#endif //Hi-843B OTP function finish
