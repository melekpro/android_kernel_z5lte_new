...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.linelength  = 9392,				//record different mode's linelength
		.framelength = 1608,			//record different mode's framelength
	...
	.cap = {
		.pclk = 480000000,				//record different mode's pclk
		.linelength  = 9392,				//record different mode's linelength
		.framelength = 3405,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
  ...
  .mipi_settle_delay_mode = 1,//MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL
...
#define S5K3L8_USE_AWB_OTP
#if defined(S5K3L8_USE_AWB_OTP)
/*
extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

#define S5K3L8_WRITE_ID 0x20
#define S5K3L8_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, S5K3L8_WRITE_ID)

kal_uint16 S5K3L8_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,S5K3L8_WRITE_ID);
    return get_byte;
}
*/
...
#define RG_TYPICAL 0x210
#define BG_TYPICAL 0x1ed
...
bool S5K3L8_read_otp_wb(struct S5K3L8_MIPI_otp_struct *otp)
{
	kal_uint32 R_to_G=0, B_to_G=0;
	kal_uint16 awbflag=0;

	//write_cmos_sensor(0x0136, 0x1800); //24MHZ
	//write_cmos_sensor(0x0304, 0x0006); //PLL pre div
	//write_cmos_sensor(0x0306, 0x00AF); //pll multip lier
	//write_cmos_sensor(0x0302, 0x0001); //vt_sys_clk_div
	//write_cmos_sensor(0x0300, 0x0005); //vt_pix_clk_div
	//write_cmos_sensor(0x030C, 0x0006); //second_pre_pll_clk_div
	//write_cmos_sensor(0x030E, 0x00AF); //second_pll_multiplier
	//write_cmos_sensor(0x030A, 0x0001); //op_sys_clk_div
	//write_cmos_sensor(0x0308, 0x0008); //op_pix_clk_div
	
	write_cmos_sensor(0x0100, 0x0100); //Streaming on	
	write_cmos_sensor(0x0A02, 0x0100); //page set 1
	write_cmos_sensor(0x0A00, 0x0100); //otp enable read
	mdelay(5);
	awbflag = read_cmos_sensor_byte(0x0A0E);
	LOG_INF("awbflag=[%d]\n", awbflag);	
	write_cmos_sensor(0x0A00, 0x0000); //otp disable read
	if(awbflag==0xD0)
	{
		write_cmos_sensor(0x0100, 0x0100); //Streaming on	
		write_cmos_sensor(0x0A02, 0x0100); //page set 1
		write_cmos_sensor(0x0A00, 0x0100); //otp enable read
		mdelay(5);
		R_to_G = (read_cmos_sensor_byte(0x0A1C)<<8)+read_cmos_sensor_byte(0x0A1D);
		B_to_G = (read_cmos_sensor_byte(0x0A1E)<<8)+read_cmos_sensor_byte(0x0A1F);
		write_cmos_sensor(0x0A00, 0x0000); //otp disable read
	}
	
	else if (awbflag==0x40)
	{
		write_cmos_sensor(0x0100, 0x0100); //Streaming on	
		write_cmos_sensor(0x0A02, 0x0100); //page set 1
		write_cmos_sensor(0x0A00, 0x0100); //otp enable read
		mdelay(5);
		R_to_G = (read_cmos_sensor_byte(0x0A10)<<8)+read_cmos_sensor_byte(0x0A11);
		B_to_G = (read_cmos_sensor_byte(0x0A12)<<8)+read_cmos_sensor_byte(0x0A13);
		write_cmos_sensor(0x0A00, 0x0000); //otp disable read
	}
	else
	{
		LOG_INF("[S5K3L8_read_otp_wb] read otp wb err! awbflag =[%d]\n",awbflag);
		return false;
	}	
	...
	if((R_to_G != 0) && (B_to_G != 0))
	{
		otp->R_to_G = R_to_G;
		otp->B_to_G = B_to_G;
		LOG_INF("  otp->R_to_G=0x%x\n", otp->R_to_G);	
		LOG_INF("  otp->B_to_G=0x%x\n", otp->B_to_G);	
	}	
	return true;
}

void S5K3L8_algorithm_otp_wb1(struct S5K3L8_MIPI_otp_struct *otp)
{
...
void S5K3L8_write_otp_wb(struct S5K3L8_MIPI_otp_struct *otp)
{
	...
	LOG_INF("R_GainH=0x%x\n", R_GainH);	
	LOG_INF("R_GainL=0x%x\n", R_GainL);	
	LOG_INF("B_GainH=0x%x\n", B_GainH);
	LOG_INF("B_GainL=0x%x\n", B_GainL);	
	LOG_INF("G_GainH=0x%x\n", G_GainH);	
	LOG_INF("G_GainL=0x%x\n", G_GainL);

	write_cmos_sensor(0x6028, 0x4000);//change to page 4000
	write_cmos_sensor_byte(0x020e, G_GainH);
	write_cmos_sensor_byte(0x020f, G_GainL);
	write_cmos_sensor_byte(0x0210, R_GainH);
	write_cmos_sensor_byte(0x0211, R_GainL);
	write_cmos_sensor_byte(0x0212, B_GainH);
	write_cmos_sensor_byte(0x0213, B_GainL);
	write_cmos_sensor_byte(0x0214, G_GainH);
	write_cmos_sensor_byte(0x0215, G_GainL);

	LOG_INF("[0x020e,0x%x]\n", read_cmos_sensor_byte(0x020e));	
	LOG_INF("[0x020f,0x%x]\n", read_cmos_sensor_byte(0x020f));	
	LOG_INF("[0x0210,0x%x]\n", read_cmos_sensor_byte(0x0210));
	LOG_INF("[0x0211,0x%x]\n", read_cmos_sensor_byte(0x0211));	
	LOG_INF("[0x0212,0x%x]\n", read_cmos_sensor_byte(0x0212));	
	LOG_INF("[0x0213,0x%x]\n", read_cmos_sensor_byte(0x0213));
	LOG_INF("[0x0214,0x%x]\n", read_cmos_sensor_byte(0x0214));	
	LOG_INF("[0x0215,0x%x]\n", read_cmos_sensor_byte(0x0215));
}

kal_uint16 S5K3L8_update_wb_register_from_otp(void)
{
	struct S5K3L8_MIPI_otp_struct current_otp;
	if(S5K3L8_read_otp_wb(&current_otp))
	{
		S5K3L8_algorithm_otp_wb1(&current_otp);
		S5K3L8_write_otp_wb(&current_otp);
	}
	return 0;
}
#endif
...
static void sensor_init(void)
{
  LOG_INF("E\n");
	...
	write_cmos_sensor_byte(0xB0A0, 0x7D);
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x0342, 0x24B0);
  write_cmos_sensor(0x0340, 0x0648);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x0CF4);
  write_cmos_sensor_byte(0x6F12, 0x01);
  write_cmos_sensor(0x6028, 0x4000);
  write_cmos_sensor_byte(0x0B00, 0x01);
  write_cmos_sensor(0x316a, 0x00A0);
  write_cmos_sensor(0x6028, 0x4000);
  write_cmos_sensor(0x0100, 0x0100);
  write_cmos_sensor_byte(0x305A, 0x08);
}	/*	preview_setting  */
...
static void capture_setting(void)
{
  ...
  write_cmos_sensor(0x0308, 0x0008);
  write_cmos_sensor(0x0342, 0x24B0);
  write_cmos_sensor(0x0340, 0x0D4D);
  write_cmos_sensor(0x0202, 0x0200);
  write_cmos_sensor(0x0200, 0x00C6);
  write_cmos_sensor(0x0B04, 0x0101);
  write_cmos_sensor(0x0B08, 0x0000);
...
#ifdef S5K3L8_USE_AWB_OTP
	S5K3L8_update_wb_register_from_otp();
#endif	
	return ERROR_NONE;
}	/*	preview   */
