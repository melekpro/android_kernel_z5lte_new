...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .linelength = 2416,                //record different mode's linelength
...
static void preview_setting(void)
{
	...
	/*ISP*/  
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, 0x02); //crop y                                       
	write_cmos_sensor(0x94, 0x03); //crop x  
	write_cmos_sensor(0x95, 0x03); //972
	write_cmos_sensor(0x96, 0xcc);
	write_cmos_sensor(0x97, 0x05); //1296
	write_cmos_sensor(0x98, 0x10); 	
       /*BLK*/
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x4e, 0x00);
	write_cmos_sensor(0x4f, 0xC3); 
	write_cmos_sensor(0xfe, 0x00);
	/*MIPI*/																		
	write_cmos_sensor(0xfe, 0x03);						
	...
	write_cmos_sensor(0x21, 0x10);
	write_cmos_sensor(0x22, 0x02);//05
	write_cmos_sensor(0x23, 0x10);//30
	write_cmos_sensor(0x24, 0x02);
	write_cmos_sensor(0x25, 0x12);//16
	write_cmos_sensor(0x26, 0x04);//09
	write_cmos_sensor(0x2a, 0x0a);//0d
	write_cmos_sensor(0x2b, 0x04);//09
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor(0x08, 0x10);
	write_cmos_sensor(0x0a, 0x14);						   
	write_cmos_sensor(0x0c, 0x10);//08 betty
	write_cmos_sensor(0x0d, 0x07); 
	write_cmos_sensor(0x0e, 0xa0); 
	write_cmos_sensor(0x0f, 0x0a); 
	write_cmos_sensor(0x10, 0x30);//44 betty
	write_cmos_sensor(0x29, Val28[BinorFull][gainlevel]);
	write_cmos_sensor(0xe3, 0x00);
	/*ISP*/  
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, 0x04); //crop y                                       
	write_cmos_sensor(0x94, 0x05); //crop x  
	write_cmos_sensor(0x95, 0x07); 
	write_cmos_sensor(0x96, 0x98);
	write_cmos_sensor(0x97, 0x0a); 
	write_cmos_sensor(0x98, 0x20); 
