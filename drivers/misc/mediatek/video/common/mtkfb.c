...
//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]start
#include <linux/seq_file.h>
#include <linux/proc_fs.h> 
//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]end
...
//+++++ lihua for lcm rst control start
static const char *this_state_name[3] = {
    "lcm_rst_gpio_state_l",
    "lcm_rst_gpio_state_h",
    "default"
};
...
static int lcm_rst_gpio_init(struct platform_device *pdev)
{
	long ret = 0;
...
long lcm_rst_gpio_select_state(int s)
{
	return _set_state(this_state_name[s]);
}

//+++++ lihua for lcm rst control end
...
//+++++ lihua for lcm rst control start
static int mtkfb_lcm_rst_probe(struct platform_device *dev)
{
    lcm_rst_gpio_init(dev);
    return 0;
}

#define MTKFB__LCM_RST_DRIVER "mtkfb_lcm_rst"
static const struct of_device_id mtkfb_lcm_rst_of_ids[] = {
    {.compatible = "mediatek,lcm_rst",},
	{}
};

static struct platform_driver mtkfb_lcm_rst_driver = {
    .probe = mtkfb_lcm_rst_probe,
	.driver = {
		.name = MTKFB__LCM_RST_DRIVER,
		.of_match_table = mtkfb_lcm_rst_of_ids,
	},
};
...
static void wtk_lcm_command_line(void)
{
	char *p, *q;
...
static const struct file_operations wtk_lcm_info_proc_fops = {	
	.owner      = THIS_MODULE,
...
{
	struct proc_dir_entry *lcm_info_entry = proc_create("wtk_lcdInfo", 0444, NULL, &wtk_lcm_info_proc_fops);
...
int __init mtkfb_init(void)
{
    ...
    //+++++ lihua for lcm rst control start
    if (platform_driver_register(&mtkfb_lcm_rst_driver)) {
		PRNERR("failed to register mtkfb driver\n");
		r = -ENODEV;
		goto exit;
	}
	...
	mtkfb_ipo_init();
#if 1//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]start
	wtk_creat_proc_lcm_info();
	wtk_lcm_command_line();
#endif//lisong 2014-12-15 [BUGID:NULL][add for *#*#9375#*#*]end
exit:
	MSG_FUNC_LEAVE();
	return r;
...
static void __exit mtkfb_cleanup(void)
{
    ...
    //+++++ lihua for lcm rst control start
    platform_driver_unregister(&mtkfb_lcm_rst_driver);
    //+++++ lihua for lcm rst control end
