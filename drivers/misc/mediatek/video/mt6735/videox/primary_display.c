...
#if 1//lisong 2016-5-20 [BUGID:NULL][fix bug:lcm flash in ata check]start    
	{
		LCM_DRIVER *lcm_drv = NULL;

		if (pgc->plcm->params && pgc->plcm->drv) {
			lcm_drv = pgc->plcm->drv;
			if (lcm_drv->ata_check_custom) {
				if(lcm_drv->ata_check_custom() == 1){
					DISPCHECK("[ATA_LCM]ata_check_custom success!!\n");
					ret = 1;
					goto done;
				}
			} 
...
#endif//lisong 2016-5-20 [BUGID:NULL][fix bug:lcm flash in ata check]end
