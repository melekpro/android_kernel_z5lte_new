...
static void __iomem *ddp_apsleep_base;

#ifndef AP_PLL_CON0
#define AP_PLL_CON0 (ddp_apmixed_base + 0x00)
#endif
#ifndef AP_SLEEP_DIS_PWR_CON
#define AP_SLEEP_DIS_PWR_CON (ddp_apsleep_base + 0x23C)
#endif
...
static DSI_STATUS DSI_Reset(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	if (cmdq)
		DDPMSG("DSI_RESET Protect may not work!/n");
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPMSG("DSI_RESET backup dsi%d irq:0x%08x ", i, irq_en[i]);
	}
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPMSG("DSI_RESET restore dsi%d irq:0x%08x ", i,
			 ...
			 unsigned int type, unsigned int enable, unsigned int skip_num)
{
	...
	DDPMSG("module=%d,mode=%d,type=%d,enable=%d,skip_num=%d\n", module, mode, type,
		 enable, skip_num);
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
				    ...
				    LCM_DSI_PARAMS *dsi_params)
{
	...
	bool hstx_cklp_en = true;//false;modify by yangjuwei for esd
...
/*	unsigned int fbk_div = 0;*/
	...
	if (0 != dsi_params->PLL_CLOCK) {
		...
		DDPMSG("DISP/DSI DSI_PHY_TIMCONFIG, Cycle Time = %d(ns), Unit Interval = %d(ns), lane# = %d\n",
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
	...
	DDPMSG("DSI_PHY_TIMCONFIG, HS_TRAIL = %d, HS_ZERO = %d, HS_PRPR = %d, LPX = %d, TA_GET = %d\n",
		 timcon0.HS_TRAIL, timcon0.HS_ZERO, timcon0.HS_PRPR, timcon0.LPX, timcon1.TA_GET);
	DDPMSG("TA_SURE = %d, TA_GO = %d, CLK_TRAIL = %d, CLK_ZERO = %d, CLK_HS_PRPR = %d\n",
...
/*	DISPFUNC();*/
	...
	if (!node)
		DDPERR("[DDP_APSLEEP] DISP find sleep node failed\n");

	ddp_apsleep_base = of_iomap(node, 0);
	if (!ddp_apsleep_base)
		DDPERR("[DDP_APSLEEP] DISP sleep base failed\n");
#endif /* CONFIG_MTK_CLKMGR */
	DISPFUNC();
...
#else
	{
		...
		if (module == DISP_MODULE_DSI1 || module == DISP_MODULE_DSIDUAL) {
			...
			if (ret > 0)
				DDPERR("DSI1 power manager API return false\n");
...
int ddp_dsi_power_on(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (!s_isDsiPowerOn) {
		...
		if (module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSIDUAL) {
			...
			ret += ddp_clk_enable(DISP1_DSI_ENGINE);
			ret += ddp_clk_enable(DISP1_DSI_DIGITAL);
			pr_warn("%s HW_DCM_DIS1 CON:0x%x, DIS_PWR_CON:0x%x\n", __func__,
				INREG32(DISP_REG_CONFIG_MMSYS_HW_DCM_DIS0),
				INREG32(AP_SLEEP_DIS_PWR_CON));
#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
    ...
    unsigned int data_arr[4] = {0,0,0,0};
    static unsigned int data_paras[2] = {0x0f,0xf0};
    unsigned char packet_type;
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
            ...
            printk("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",
			    ...
			    ("[DSI]cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",
			     ...
			     dsi_params->lcm_esd_check_table[i].para_list[1]);
                       ...
                       if(packet_type == 0x1A || packet_type == 0x1C)
                       {
                          ...
                          data_arr[0] = read_data1.byte0;
                          data_arr[1] = read_data1.byte1;
                          data_arr[2] = read_data1.byte2;
                          data_arr[3] = read_data1.byte3;
                          j = 0;
                          while(recv_data_cnt--){
                          if(i!=2)
                          {
                              if(data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]){
                                         ret = 0; // esd pass
                                          printk("[DSI]lsm-debug1-j: %d ,i: %d ,ret = %d \n",j,i,ret);
                                  }
                                 else{
                                         ret = 1; // esd fail
                                           printk("[DSI]lsm-debug2-j: %d ,i: %d ,ret = %d \n",j,i,ret);
                                         break;
                                 }                                        
                          }
                          else{
                              if((data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]) || (data_arr[j] == data_paras[j])){
                                         ret = 0; // esd pass
                                           printk("[DSI]lsm-debug3-j: %d ,i: %d ,ret = %d \n",j,i,ret);
                              }
                              else{
                                         ret = 1; // esd fail
                                           printk("[DSI]lsm-debug4-j: %d ,i: %d ,ret = %d \n",j,i,ret);
                                         break;                                          
                              }
                          }
...
/* hsa_byte = (dsi_params->horizontal_sync_active * fbconfig_dsiTmpBufBpp - 10); */
	...
	case SSC_EN:
		...
		break;
	default:
		DDPERR("fbconfig dsi set timing :no such type!!\n");
	}
}
