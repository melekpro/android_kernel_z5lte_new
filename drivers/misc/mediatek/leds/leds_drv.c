...
#if 1//lisong 2016-7-14[BUGID:NULL][led brightness test]start
static struct class *led_test_class;

static unsigned int mt6328_reg;
static unsigned int mt6328_data;
static ssize_t led_test_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	int ret;	

    ret = sscanf(buf, "%d,%d", &mt6328_reg, &mt6328_data);

    pmic_set_register_value(mt6328_reg, mt6328_data);
    
	return count; 
}

static ssize_t led_test_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "reg[%d]=%d, isink0[%d,%d,%d], isink1[%d,%d,%d]\n",
                        mt6328_reg,mt6328_data,
                        PMIC_ISINK_CH0_MODE,PMIC_ISINK_CH0_STEP,PMIC_ISINK_DIM0_DUTY,
                        PMIC_ISINK_CH1_MODE,PMIC_ISINK_CH1_STEP,PMIC_ISINK_DIM1_DUTY);    
}

static CLASS_ATTR(led_test, S_IWUSR | S_IRUGO, led_test_show, led_test_store);
#endif//lisong 2016-7-14[BUGID:NULL][led brightness test]end
...
#if 1//lisong 2016-7-14[BUGID:NULL][led brightness test]start
	led_test_class = class_create(THIS_MODULE, "led_test_class");
	ret = class_create_file(led_test_class, &class_attr_led_test);
	if (ret) {
		printk("class_create_file led_test_class ERROR\n");
	}
#endif//lisong 2016-7-14[BUGID:NULL][led brightness test]end
