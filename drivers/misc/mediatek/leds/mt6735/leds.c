...
#if defined(CONFIG_WTK_LEDS_AW2015) || defined(CONFIG_WTK_LEDS_AW2013)
extern ssize_t external_ic_leds_mode(int value);//add by yangjuwei 20160219
#endif
...
#ifdef CONFIG_ISINK_BREATH_MODE
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	pmic_type!= MT65XX_LED_EXTERNAL_IC_0 && pmic_type!= MT65XX_LED_EXTERNAL_IC_1 && pmic_type!= MT65XX_LED_EXTERNAL_IC_2) || 
	led->nled_mode != NLED_BLINK) {
		return -1;
	}
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
		...
		pmic_set_register_value(PMIC_ISINK_CH1_EN, NLED_ON);
		break;
	//add by yangjuwei 20160219 start
	#if defined(CONFIG_WTK_LEDS_AW2015) || defined(CONFIG_WTK_LEDS_AW2013)
	case MT65XX_LED_EXTERNAL_IC_0:
		external_ic_leds_mode(4);
		break;
	case MT65XX_LED_EXTERNAL_IC_1:
		external_ic_leds_mode(5);
		break;
	case MT65XX_LED_EXTERNAL_IC_2:
		external_ic_leds_mode(6);
		break;
	#endif
		//add by yangjuwei 20160219 end
	default:
		break;
	}
	return 0;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	//add by yangjuwei 20160219 start
	#if defined(CONFIG_WTK_LEDS_AW2015) || defined(CONFIG_WTK_LEDS_AW2013)
	else if(pmic_type == MT65XX_LED_EXTERNAL_IC_0)
	{
		if (level)
			external_ic_leds_mode(1);
		else
			external_ic_leds_mode(0);
		mutex_unlock(&leds_pmic_mutex);
		return 0;
	}
	else if(pmic_type == MT65XX_LED_EXTERNAL_IC_1)
	{
		
		if (level)
			external_ic_leds_mode(2);
		else
			external_ic_leds_mode(0);
		mutex_unlock(&leds_pmic_mutex);
		return 0;
	}
	else if(pmic_type == MT65XX_LED_EXTERNAL_IC_2)
	{
		if (level)
			external_ic_leds_mode(3);
		else
			external_ic_leds_mode(0);
		mutex_unlock(&leds_pmic_mutex);
		return 0;
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_BLS_PWM:
		...
		//add by yangjuwei 20160219 start
	#if defined(CONFIG_WTK_LEDS_AW2015) || defined(CONFIG_WTK_LEDS_AW2013)
	case MT65XX_LED_MODE_EXTERNAL_IC:
			...
			unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (*delay_on != led_data->delay_on
		...
		if (led_data->delay_on && led_data->delay_off) {	/* enable blink */
			...
			#if defined(CONFIG_WTK_LEDS_AW2015) || defined(CONFIG_WTK_LEDS_AW2013)
			else if(led_data->cust.mode == MT65XX_LED_MODE_EXTERNAL_IC)
			{
				LEDS_DEBUG("MT65XX_LED_MODE_EXTERNAL_IC on:%s,%d\n",__func__,__LINE__);
				nled_tmp_setting.nled_mode = NLED_BLINK;
			...
			{
				LEDS_DEBUG("MT65XX_LED_MODE_EXTERNAL_IC off:%s,%d\n",__func__,__LINE__);
