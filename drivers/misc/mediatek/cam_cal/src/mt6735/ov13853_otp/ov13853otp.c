...
#define CAM_CALERR(fmt, arg...)    pr_err(PFX "[%s] " fmt, __func__, ##arg)
	...
	for(i=0;i<buffersize;i++)
		{
			...
			*(iBuffer+buffersize-1-i)=readbuff;
		      CAM_CALDB("huyuan read af data[%d]=0x%x\n",(buffersize-1-i),iBuffer[buffersize-1-i]);
		}
...
//for(j=0;j<1868;j++)
// 	{
//		printk("wuhao read_lsc_otp reg=%d, data=%d\n",j,OTPData[j]);
// 	}
#if 1
 if(otp_flag){
...
static long imx214otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = imx214otp_Ioctl_Compat,
#endif
...
struct of_device_id otp_of_match[] = {
...
#ifdef CONFIG_OF
static const struct of_device_id ov13853otp_of_match[] = {
...
static struct platform_driver g_stCAM_CAL_Driver = {
    ...
    .driver		= {
			        ...
			        .owner	= THIS_MODULE,
				#ifdef CONFIG_OF
					.of_match_table = ov13853otp_of_match,
				#endif        
   			 },
};
...
{
	CAM_CALDB("Start CAM_CAL_i2C_init ov13853_otp\n");
