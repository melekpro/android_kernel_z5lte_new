...
#if 1//lihua add for FM switch FM start
extern void accdet_switch_fm_gpio_select(int val);
static ssize_t store_switch_FM_state(struct device_driver *ddri, const char *buf, size_t count)
{
	int ret;
    int switch_FM_value;

	ret = sscanf(buf, "%d", &switch_FM_value);
    printk("[Accdet_switch_FM status: %d]\n", switch_FM_value);
    if (switch_FM_value)
    {
        printk("[Accdet_switch_FM enable]\n");
        accdet_switch_fm_gpio_select(1);
    }
    else
    {
        printk("[Accdet_switch_FM disable]\n");
        accdet_switch_fm_gpio_select(0);
    }

	return count;
}
#endif//lihua add for switch FM end

//add by yangjuwei 20160706 start
#ifdef CONFIG_WTK_CLOSE_ACCDET
extern int switch_accdet_value;
extern int ext_amp_on_flag;
static ssize_t store_switch_accdet_state(struct device_driver *ddri, const char *buf, size_t count)
{
	int ret;

	ret = sscanf(buf, "%d", &switch_accdet_value);
    printk("[Accdet_switch_FM status: %d]\n", switch_accdet_value);
	
    if (switch_accdet_value&&ext_amp_on_flag)
    {
        printk("[Accdet_switch_FM enable]\n");
        accdet_switch_fm_gpio_select(1);
    }
...
static DRIVER_ATTR(switch_accdet_state, 0664, NULL, store_switch_accdet_state);
//add by yangjuwei 20160706 end
#endif
...
#if 1//lihua add for FM switch FM start
static DRIVER_ATTR(switch_FM_state, 0664, NULL, store_switch_FM_state);
#endif//lihua add for switch FM end 
...
#if 1//lihua add for FM switch FM start	
	&driver_attr_switch_FM_state,
#endif//lihua add for FM switch FM end	
	/*#endif*/
#if defined(CONFIG_TS3A225E_ACCDET)
...
//add by yangjuwei 20160706 start
#ifdef CONFIG_WTK_CLOSE_ACCDET
	&driver_attr_switch_accdet_state,
#endif
//add by yangjuwei 20160706 end
};
