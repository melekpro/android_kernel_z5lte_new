...
struct alsps_hw *get_alsps_dts_func(const char *name, struct alsps_hw *hw)
{
	...
	ret = of_property_read_u32_array(node , "ps_integration_time", ps_integration_time, ARRAY_SIZE(ps_integration_time));
	if (ret == 0)
		hw->ps_integration_time		 = ps_integration_time[0];
