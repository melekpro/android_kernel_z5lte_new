...
extern  int KTD2685_write_reg(u8 reg, u8 val);
extern  int KTD2685_read_reg(u8 reg);
...
#if 0//lisong 2016-1-18[BUGID:XELLWM-1515][fix bug: flashlight can not work if battery soc below 15%]start 
...
#endif//lisong 2016-1-18[BUGID:XELLWM-1515][fix bug: flashlight can not work if battery soc below 15%]end
...
static const struct of_device_id flashlight_custom_of_match[] = {
...
enum fl_gpio_type {
	GPIO_DEFAULT = 0,
	GPIO_MAIN_KTD265_EN_LOW,
	GPIO_MAIN_KTD265_EN_HIGH,
	GPIO_MAIN_KTD265_MODE_LOW,
	GPIO_MAIN_KTD265_MODE_HIGH,
	GPIO_MAIN_WD3124DB_EN_LOW,
	GPIO_MAIN_WD3124DB_EN_HIGH,
	GPIO_MAIN_WD3124DB_FLASH_LOW,
	GPIO_MAIN_WD3124DB_FLASH_HIGH,
	GPIO_MAIN_KTD2685_HWEN_LOW,
	GPIO_MAIN_KTD2685_HWEN_HIGH,
	GPIO_MAIN_FAKE_EN_LOW,
	GPIO_MAIN_FAKE_EN_HIGH,
	GPIO_SUB_FAKE_EN_LOW,
	GPIO_SUB_FAKE_EN_HIGH,
	GPIO_NUM
};

struct pinctrl *fl_pinctrl = NULL;
...
static struct fl_gpio_attr fl_gpios[GPIO_NUM] = {
	[GPIO_DEFAULT] = {"default", false, NULL},
	[GPIO_MAIN_KTD265_EN_LOW] = {"main_ktd265_en_pin_low", false, NULL},
	[GPIO_MAIN_KTD265_EN_HIGH] = {"main_ktd265_en_pin_high", false, NULL},
	[GPIO_MAIN_KTD265_MODE_LOW] = {"main_ktd265_mode_pin_low", false, NULL},
	[GPIO_MAIN_KTD265_MODE_HIGH] = {"main_ktd265_mode_pin_high", false, NULL},
	[GPIO_MAIN_WD3124DB_EN_LOW] = {"main_wd3124db_en_pin_low", false, NULL},
	[GPIO_MAIN_WD3124DB_EN_HIGH] = {"main_wd3124db_en_pin_high", false, NULL},
	[GPIO_MAIN_WD3124DB_FLASH_LOW] = {"main_wd3124db_flash_pin_low", false, NULL},
	[GPIO_MAIN_WD3124DB_FLASH_HIGH] = {"main_wd3124db_flash_pin_high", false, NULL},
	[GPIO_MAIN_KTD2685_HWEN_LOW] = {"main_ktd2685_hwen_pin_low", false, NULL},
	[GPIO_MAIN_KTD2685_HWEN_HIGH] = {"main_ktd2685_hwen_pin_high", false, NULL},
	[GPIO_MAIN_FAKE_EN_LOW] = {"main_fake_en_pin_low", false, NULL},
	[GPIO_MAIN_FAKE_EN_HIGH] = {"main_fake_en_pin_high", false, NULL},
	[GPIO_SUB_FAKE_EN_LOW] = {"sub_fake_en_pin_low", false, NULL},
	[GPIO_SUB_FAKE_EN_HIGH] = {"sub_fake_en_pin_high", false, NULL},
	};

static unsigned int fl_main_support = 0;
static unsigned int fl_sub_support = 0;
static unsigned int fl_main_mode = 0;
static unsigned int fl_sub_mode = 0;

void flashlight_custom_init(struct platform_device *pdev)
{
    int ret;
    int i = 0;
    unsigned int data1[2]={0, 0};
    unsigned int data2[2]={0, 0};
    struct device_node *node = NULL; 

    fl_pinctrl = devm_pinctrl_get(&pdev->dev);
    if (IS_ERR(fl_pinctrl)) {
        ret = PTR_ERR(fl_pinctrl);
        return;
    }

    for (i = 0; i < ARRAY_SIZE(fl_gpios); i++) {
        fl_gpios[i].gpioctrl = pinctrl_lookup_state(fl_pinctrl, fl_gpios[i].name);
        if (IS_ERR(fl_gpios[i].gpioctrl)) {
            ret = PTR_ERR(fl_gpios[i].gpioctrl);
        } else {
            fl_gpios[i].gpio_prepare = true;
        }
    }

    node = of_find_matching_node(node, flashlight_custom_of_match);
    if (node) {
        of_property_read_u32_array(node, "flashlight_main_support", data1, ARRAY_SIZE(data1));
        of_property_read_u32_array(node, "flashlight_sub_support", data2, ARRAY_SIZE(data2));
        fl_main_support = data1[0];
        fl_main_mode = data1[1];
        fl_sub_support = data2[0];
        fl_sub_mode = data2[1];
    }
}

void flashlight_custom_set(fl_dev dev, fl_mode mode)
{
    if(dev == FL_DEV_MAIN)
    {
        if(fl_main_support == 0)
            return;
        ...
        switch(fl_main_mode)
        {
            case 0:/*GPIO MODE*/
                if(mode == FL_MODE_OFF)
                {
                    if (fl_gpios[GPIO_MAIN_FAKE_EN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_FAKE_EN_LOW].gpioctrl);

                }
                else
                {
                    if (fl_gpios[GPIO_MAIN_FAKE_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_FAKE_EN_HIGH].gpioctrl);
                }
                break;
            case 1:/*KTD265 MODE*/
                if(mode == FL_MODE_FLASH)
                {
                    if (fl_gpios[GPIO_MAIN_KTD265_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_EN_HIGH].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_KTD265_MODE_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_MODE_LOW].gpioctrl);
                }
                else if(mode == FL_MODE_TORCH)
                {
                    if (fl_gpios[GPIO_MAIN_KTD265_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_EN_HIGH].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_KTD265_MODE_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_MODE_HIGH].gpioctrl);
                }
                else
                {
                    if (fl_gpios[GPIO_MAIN_KTD265_EN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_EN_LOW].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_KTD265_MODE_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_MODE_LOW].gpioctrl);
                }
                break;
            case 2:/*PMIC MODE*/
                ...
                break;
            case 3:/*WD3124DB MODE*/
                if(mode == FL_MODE_FLASH)
                {
                    if (fl_gpios[GPIO_MAIN_WD3124DB_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_EN_HIGH].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_WD3124DB_FLASH_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_FLASH_HIGH].gpioctrl);
                }
                else if(mode == FL_MODE_TORCH)
                {
                    if (fl_gpios[GPIO_MAIN_WD3124DB_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_EN_HIGH].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_WD3124DB_FLASH_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_FLASH_LOW].gpioctrl);
                }
                else
                {
                    if (fl_gpios[GPIO_MAIN_WD3124DB_EN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_EN_LOW].gpioctrl);
                    if (fl_gpios[GPIO_MAIN_WD3124DB_FLASH_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_WD3124DB_FLASH_LOW].gpioctrl);
                }
                break;
            ...
            case 4:/*KTD2685 MODE*/
                if(mode == FL_MODE_FLASH)
                {
                    if (fl_gpios[GPIO_MAIN_KTD2685_HWEN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_HWEN_HIGH].gpioctrl);
//                    if (fl_gpios[GPIO_MAIN_KTD2685_TORCH_LOW].gpio_prepare)
//                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_TORCH_LOW].gpioctrl);
//                    if (fl_gpios[GPIO_MAIN_KTD2685_STROBE_HIGH].gpio_prepare)
//                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_STROBE_HIGH].gpioctrl);
			
                    KTD2685_write_reg(0x03,0x00);
                    KTD2685_write_reg(0x04,0x00);
                    KTD2685_write_reg(0x01,0x2F);
                }
                else if(mode == FL_MODE_TORCH)
                {
                    if (fl_gpios[GPIO_MAIN_KTD2685_HWEN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_HWEN_HIGH].gpioctrl);
//                    if (fl_gpios[GPIO_MAIN_KTD2685_STROBE_LOW].gpio_prepare)
//                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_STROBE_LOW].gpioctrl);
//                    if (fl_gpios[GPIO_MAIN_KTD2685_TORCH_HIGH].gpio_prepare)
//                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_TORCH_HIGH].gpioctrl);
		
                    KTD2685_write_reg(0x05,0x00);
                    KTD2685_write_reg(0x06,0x00);
                    KTD2685_write_reg(0x01,0x1B);
                }
                else
                {
                    if (fl_gpios[GPIO_MAIN_KTD2685_HWEN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD2685_HWEN_LOW].gpioctrl);
                    
                    KTD2685_write_reg(0x01,0x00);
                }
                break;        
            case 5:/*KTD265 TORCH MODE*/
                if(mode == FL_MODE_OFF)
                {
                    if (fl_gpios[GPIO_MAIN_KTD265_EN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_MAIN_KTD265_EN_LOW].gpioctrl);
    ...
    else
    {
        ...
        switch(fl_sub_mode)
        {
            case 0:/*GPIO MODE*/
                if(mode == FL_MODE_OFF)
                {
                    if (fl_gpios[GPIO_SUB_FAKE_EN_LOW].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_SUB_FAKE_EN_LOW].gpioctrl);
                }
                else
                {
                    if (fl_gpios[GPIO_SUB_FAKE_EN_HIGH].gpio_prepare)
                        pinctrl_select_state(fl_pinctrl, fl_gpios[GPIO_SUB_FAKE_EN_HIGH].gpioctrl);
                }
                break;
            case 1:/*PMIC MODE*/
                if(mode == FL_MODE_OFF)
...
#if 1//lihua[2015-12-28][BUGID:NULL][SUB FLASHLIGHT CUSTOM SUPPORT IN FACTORY MODE]start
static struct class *sub_flashlight_class;
static struct device *sub_flashlight_device;

static ssize_t sub_flashlight_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}
...
static DEVICE_ATTR(sub_flashlight_enable, S_IWUSR | S_IRUGO, sub_flashlight_enable_show,
		   sub_flashlight_enable_store);
#endif//lihua[2015-12-28][BUGID:NULL][SUB FLASHLIGHT CUSTOM SUPPORT IN FACTORY MODE]end
/* ======================================================================== */
#define ALLOC_DEVNO
...
#if 1//lisong[2015-12-10][BUGID:NULL][FLASHLIGHT CUSTOM SUPPORT]start
	flashlight_custom_init(dev);
#endif//lisong[2015-12-10][BUGID:NULL][FLASHLIGHT CUSTOM SUPPORT]end

#if 1//lihua[2015-12-28][BUGID:NULL][SUB FLASHLIGHT CUSTOM SUPPORT IN FACTORY MODE]start
    sub_flashlight_class = class_create(THIS_MODULE, "sub_flashlight");
    if (IS_ERR(sub_flashlight_class)) {
        logI("[flashlight_probe] Unable to create class, err = %d ~",
             (int)PTR_ERR(sub_flashlight_class));
        goto flashlight_probe_error;
    }

    sub_flashlight_device =
        device_create(sub_flashlight_class, NULL, 0, NULL, "sub_flashlight");
    if (NULL == sub_flashlight_device) {
        logI("[flashlight_probe] device_create fail ~");
        goto flashlight_probe_error;
    }

    device_create_file(sub_flashlight_device, &dev_attr_sub_flashlight_enable);
#endif//lihua[2015-12-28][BUGID:NULL][SUB FLASHLIGHT CUSTOM SUPPORT IN FACTORY MODE]end
...
#if 1//lisong[2015-12-10][BUGID:NULL][FLASHLIGHT CUSTOM SUPPORT]start		   
#ifdef CONFIG_OF
		   .of_match_table = flashlight_custom_of_match,
#endif		  
#endif//lisong[2015-12-10][BUGID:NULL][FLASHLIGHT CUSTOM SUPPORT]end
		   },
};

#if 0//lisong[2015-12-10][BUGID:NULL][FLASHLIGHT CUSTOM SUPPORT]start
