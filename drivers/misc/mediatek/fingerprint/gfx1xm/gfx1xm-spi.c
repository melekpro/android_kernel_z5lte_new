...
{
    irq_flag = 1;
    gfx1xm_debug(DEFAULT_DEBUG, "gfx1xm: %s  %d", __func__, __LINE__);
	//gfx1xm_eint_func();
...
static int  gfx1xm_probe(struct spi_device *spi)
{
    ...
    gfx1xm_dev->irq_gpio = -EINVAL;
    gfx1xm_dev->reset_gpio = -EINVAL;
    gfx1xm_dev->power_gpio = -EINVAL;
...
#error "have't define any product for fw"
	...
	if (minor < N_SPI_MINORS) {
		...
		gfx1xm_dev->devt = MKDEV(major_gf66xx, minor);
	...
	gfx1xm_irq_cfg(gfx1xm_dev, gfx1xm_irq);
...
error:
    ...
    if(gfx1xm_dev->buffer != NULL) {
        ...
        free_pages((unsigned long)gfx1xm_dev->buffer, get_order(bufsiz + GFX1XM_RDATA_OFFSET));
    }
 	//hwPowerDown(MT6328_POWER_LDO_VGP1, "gf66xx");
	gfx1xm_power_on(gfx1xm_dev,0);
	FUNC_EXIT();
	return status;
}
