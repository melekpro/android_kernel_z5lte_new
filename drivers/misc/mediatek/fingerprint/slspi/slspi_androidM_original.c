...
//#ifdef COMPATIBLE_SUPPORT
		...
		//daizhiling remove for build error
		//mt_eint_registration(spidev->irq, EINTF_TRIGGER_RISING, spidev_irq_routing, 1);

		mt_eint_mask(spidev->irq); //daizhiling remove
#endif
...
static int fingerprint_irq_registration(void)
{
	...
	if (node) 
	{
		...
		ret = request_irq(fingerprint_irq, (irq_handler_t)spidev_irq_routing, IRQF_TRIGGER_RISING,FINGER_PRINT_EINT_STRING, NULL);
		if (ret > 0)
		{
			ret = -1;
		}
