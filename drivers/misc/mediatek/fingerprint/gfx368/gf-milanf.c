...
#if 0
			mt_set_gpio_mode(gf_dev->irq_gpio, GF_IRQ_PIN_M_GPIO);
			mt_set_gpio_dir(gf_dev->irq_gpio, GPIO_DIR_IN); 
			mt_set_gpio_pull_enable(gf_dev->irq_gpio, GPIO_PULL_ENABLE);
			mt_set_gpio_pull_select(gf_dev->irq_gpio, GPIO_PULL_DOWN);
#else
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SENDKEY:
		...
		for(i = 0; i< ARRAY_SIZE(key_map); i++) {
            // M: none weipengfei 160616 {
            // force set report key as KEY_F19  { "CLICK",  KEY_F19 } 
            // if(key_map[i].val == gf_key.key) {
            //      input_report_key(gf_dev->input, gf_key.key, gf_key.value);
            if (key_map[i].val == KEY_F19) {
                printk("[WPF_GXFP: %s - Line:%d, key:%d, value:%d]\n", __FUNCTION__, __LINE__, gf_key.key, gf_key.value);
                input_report_key(gf_dev->input, key_map[i].val, gf_key.value);
            // M: }
...
static char gf_ic_info[16] = "GF368";
...
//	pinctrl_select_state(pinctrl1, eint_as_int);
	...
	//while(1){
		for(tt=0;tt<3;tt++) {
			...
			if(chip_id_2 == 0x0022 && chip_id_1 == 0x02A0)
			{
                ...
                strncpy(wtk_finger_info, (char*)gf_ic_info, sizeof(gf_ic_info)-1);
				//add flag for mark fingerprint initialization end
                break;
			}
	...
	if (!fingerprint_load_status)
	{
        devm_pinctrl_put(pinctrl1); //free resource
		return -EINVAL; 
	}
	//add flag for mark fingerprint initialization end
#endif
...
static int __init gf_init(void)
{
    ...
    fingerprint_spi->controller_data =&spi_conf_mt65xx;
	gf_probe(fingerprint_spi);
