...
#define   gf66xx_dbg(fmt, args...) do{ \
					printk("gf66xx:" fmt, ##args);\
				}while(0)
#define FUNC_ENTRY() do{\
					printk("gf66xx:%s, entry\n", __func__);\
					}while(0)

#define FUNC_EXIT() do{\
					printk("gf66xx:%s, exit\n", __func__);\
					}while(0)
#else
...
//#if defined(WTK_GF66XX_NODE_CONTROL_IRQ)
static struct class *gf66xx_class;
//static  bool  irq_onof=true;
static  bool  ic_check_status=false;
//#endif /*WTK_GF66XX_NODE_CONTROL_IRQ*/
...
//#if defined(WTK_GF66XX_NODE_CONTROL_IRQ)
enum{
	IC_CHECK_NO = 0,//IRQ_OFF
	IC_CHECK_YES,//IRQ_ON
};

static ssize_t enable_gf66xx_show(struct class *dev, 
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s : %d. \n", "citinfo",ic_check_status);
}

static ssize_t enable_gf66xx_store(struct class *class, 
...
static CLASS_ATTR(citinfo, 0664, enable_gf66xx_show, enable_gf66xx_store);
...
//					printk("gf66xx GF66XX_W ioc->cmd =0x%02X  ***2015032701.-lantao-\n", ioc->cmd);
			...
			/*add irq close & open issue by xuanyu.chen 150907 begin*/
            if(ioc->addr == 0x8043)
            {
	            if(gf66xx_dev->buffer[3] == 0x03) 
		        {
								gf66xx_dbg("-----lantao----- irq_close.  irq_onof = 3. irq = %d\n",gf66xx_dev->spi->irq);
								//gf66xx_irq_close(gf66xx_dev->spi->irq);//xuanyu delete for hd_fingerprint 20160319
								//gf66xx_irq_open(gf66xx_dev->spi->irq);
            } 
             else
             {
		            gf66xx_dbg("-----lantao-----  irq_open.  irq_onof = 0. irq = %d\n",gf66xx_dev->spi->irq);
								gf66xx_irq_open(gf66xx_dev->spi->irq);
             }
                        
            }
            /*add irq close & open issue by xuanyu.chen 150907 end*/

			#if 0
			if(irq_onof){
				printk("-----lantao-----  irq_open.  irq_onof = %d.\n",irq_onof);
				gf66xx_irq_open(gf66xx_dev->spi->irq);
			}
			else
			{
				printk("-----lantao----- irq_close.  irq_onof = %d.\n",irq_onof);
				gf66xx_irq_close(gf66xx_dev->spi->irq);
			}
			#endif 
		} else {
...
#else
    irq_flag = 1;
    printk("gf66xx: in irq, thread_flag = %d, suspend_flag = %d\n", thread_flag, suspend_flag);
...
{
	struct gf66xx_dev* gf66xx_dev = &gf66xx;
    struct sched_param param = {.sched_priority = 1}; //REG_RT_PRIO(1)
...
static void gf66xx_timer_work(struct work_struct *work)
{
	...
	if(value[0] == 0xC6&& value[1] == 0x47){
        ...
        if((value[0] != 0xbe) || memcmp(version, GF66XX_PID, 6)) {
            gf66xx_power_on(gf66xx_dev,0);
            mdelay(10);
            ret = gf66xx_power_on(gf66xx_dev,1);
            if(ret)
...
static void gf66xx_early_suspend(struct early_suspend *h)
{    
	struct gf66xx_dev *gf66xx_dev = container_of(h, struct gf66xx_dev, early_fp);
	gf66xx_spi_set_mode(gf66xx_dev->spi, SPEED_1MHZ, 0);	
        gf66xx_spi_write_byte(gf66xx_dev, GF66XX_MODE_STATUS, 0x03);	

    pr_info("gf66xx:gf66xx_suspend setmode 3\n");

	
}
	/*----------------------------------------------------------------------------*/
static void gf66xx_late_resume(struct early_suspend *h)
{    
	struct gf66xx_dev *gf66xx_dev = container_of(h, struct gf66xx_dev, early_fp);	
	  pr_info("gf66xx:gf66xx_late_resume\n");
}
#endif
...
//#if defined(WTK_GF66XX_NODE_CONTROL_IRQ)
		...
		if(version == 0x47)
		{
			gf66xx_class = class_create(THIS_MODULE, "gf66xx_cit_node");
			ret = class_create_file(gf66xx_class, &class_attr_citinfo);
			ic_check_status = true;	
		}
		else
		{
			pr_err("gf66xx:[ERR] %s device detect error!!\n", __func__);
			ic_check_status = false;
			status = -ENODEV;	
			goto error;			
		}	
...
#if CONFIG_HAS_EARLYSUSPEND		
gf66xx_dev->early_fp.level		= EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
gf66xx_dev->early_fp.suspend	= gf66xx_early_suspend,		
gf66xx_dev->early_fp.resume		= gf66xx_late_resume,    	
register_early_suspend(&gf66xx_dev->early_fp);
#endif
