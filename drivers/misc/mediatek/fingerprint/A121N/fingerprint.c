...
struct pinctrl_state *ma_finger_reset_high,*ma_finger_reset_low,*ma_finger_int_as_int;
...
int ma_finger_get_gpio_info(struct platform_device *pdev)
{
	...
	ma_finger_reset_high = pinctrl_lookup_state(ma_finger_pinctrl, "fp_state_rst_output1");
	...
	ma_finger_reset_low = pinctrl_lookup_state(ma_finger_pinctrl, "fp_state_rst_output0");
	...
	ma_finger_int_as_int = pinctrl_lookup_state(ma_finger_pinctrl, "fp_state_eint_as_int");
	if (IS_ERR(ma_finger_int_as_int)) {
		ret = PTR_ERR(ma_finger_int_as_int);
		dev_err(&pdev->dev, " Cannot find ma_finger pinctrl ma_finger_eint_as_int!\n");
		return ret;
	}
