...
//    unsigned int key_code;
  ...
  */      
	...
	GTP_DEBUG("--lan-- Gesture: 0x%02X, points: %d", doze_buf[0], doze_buf[1]);
	
    /* get key code */
//    key_code = ges_type < 16? KEY_GES_CUSTOM : KEY_GES_REGULAR;
	if(gesture_data.data[0] == 'c') {
	...
	else if(gesture_data.data[0] == 0x5E) {
	...
	else if(gesture_data.data[0] == 0xAA) {
	...
	else if(gesture_data.data[0] == 0xBB) {
	...
	else if(gesture_data.data[0] == 0xAB) {
	...
	else if(gesture_data.data[0] == 0xBA) {
	...
	else if(gesture_data.data[0] == 0xCC) {
