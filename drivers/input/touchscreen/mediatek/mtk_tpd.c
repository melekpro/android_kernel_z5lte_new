...
//add for can't enter meta mode without touchpanel start
#include <../../../misc/mediatek/include/mt-plat/mt_boot_common.h>
//add for can't enter meta mode without touchpanel end
...
/***********************************add for gesture start**************************************/
static struct class *tp_gesture_class;
bool tp_gesture_switch = false;
static ssize_t tp_gestrue_function_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
...
/*----------------------------------------------------------------------------*/
static ssize_t tp_gestrue_function_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "gesenable",tp_gesture_switch);    
}
static CLASS_ATTR(gesenable, S_IWUSR | S_IRUGO, tp_gestrue_function_show, tp_gestrue_function_store);
/***********************************add for gesture end**************************************/
...
//add for can't enter meta mode without touchpanel end
	...
	err = class_create_file(tp_gesture_class, &class_attr_gesenable);
	if (err) {
		printk("gensenable create sys interface ERROR\n");
	}
	return 0;
}
static int tpd_remove(struct platform_device *pdev)
...
static const struct file_operations wtk_tp_info_proc_fops = {	
	.owner      = THIS_MODULE,
...
{
	struct proc_dir_entry *tp_info_entry = proc_create("wtk_tpInfo", 0444, NULL, &wtk_tp_info_proc_fops);
