...
extern u32 mt65xx_eint_set_sens(u8 eintno, char sens);
extern void mt65xx_eint_registration(u8 eintno, char Dbounce_En, char ACT_Polarity, void (EINT_FUNC_PTR) (void), char auto_umask);
#endif
...
char module_name_g[MODULE_MAX_LEN];
char ic_g[8];

static char tp_modules[][MODULE_MAX_LEN]={
	[TEMEIKE_SENSOR_ID]="TeMeiKe",
	[HELITAI_SENSOR_ID]="HeLiTai",	
	[HELITAI_S3_SENSOR_ID]="HeLiTai",
	[LONGFEI_SENSOR_ID]="LongFei",
};

void get_module_name(int sensor_id,char *module_name)
{
	if(tp_modules[sensor_id])
		strcpy(module_name,tp_modules[sensor_id]);
	else
		strcpy(module_name,"unknow");
}
...
static int tpd_irq_registration(void)
{
	...
	if(node){
		...
		{
			ret = request_irq(tpd_touch_irq, (irq_handler_t)tpd_eint_interrupt_handler, IRQF_TRIGGER_RISING, 
			...
			ret = request_irq(tpd_touch_irq, (irq_handler_t)tpd_eint_interrupt_handler, IRQF_TRIGGER_FALLING, 
				"TOUCH_PANEL-eint", NULL);
			if(ret > 0){
			    ret = -1;
			    GTP_ERROR("tpd request_irq IRQ LINE NOT AVAILABLE!.");
...
//#endif
	...
	get_module_name(gt1x_version.sensor_id,module_name_g);
	#ifdef RGK_SAMPLE_VERSION
	len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%06X,Sample FirmWare:0x%06X",gt1x_version.product_id,module_name_g,client->addr,gt1x_version.patch_id,RGK_SAMPLE_VERSION);
	#else
	len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%06X",gt1x_version.product_id,module_name_g,client->addr,gt1x_version.patch_id);
	#endif
...
#else
extern char upmu_is_chr_det(void);
#endif
