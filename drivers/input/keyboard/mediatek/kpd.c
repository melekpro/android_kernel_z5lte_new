...
{
	kpd_suspend = true;
	//modify by chenxuanyu for vicescreen start
//#ifdef MTK_KP_WAKESOURCE
#if defined(MTK_KP_WAKESOURCE) || defined(CONFIG_WTK_SUPPORT_WAKE_KEY) || defined(MTK_KP_WAKESOURCE) || defined(CONFIG_RGK_HOME_KEY_WAKE_UP_SUPPORT) 
	if (call_status == 2) {
		kpd_print("kpd_early_suspend wake up source enable!! (%d)\n", kpd_suspend);
	} else {
        // M: XWLLE-765 daizhiyi 20150715 {
		#if defined(CONFIG_WTK_SUPPORT_WAKE_KEY) || defined(CONFIG_RGK_HOME_KEY_WAKE_UP_SUPPORT)
...
#endif
	//modify by chenxuanyu for vicescreen end
	kpd_print("suspend!! (%d)\n", kpd_suspend);
	return 0;
}
