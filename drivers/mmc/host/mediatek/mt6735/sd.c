...
#if 0//lisong[2015-12-11][BUGID:NULL][init sdcard hotplug pin state]start
static void msdc_hotplug_pin_init(struct mmc_host *mmc)
{
    struct pinctrl *hotplug_pinctrl;
    struct pinctrl_state *pins_default;
    struct pinctrl_state *pins_eint_int;
    int ret;

    hotplug_pinctrl = devm_pinctrl_get(mmc->parent);
    if (IS_ERR(hotplug_pinctrl)) {
        ret = PTR_ERR(hotplug_pinctrl);
        return ret;
    }

    pins_default = pinctrl_lookup_state(hotplug_pinctrl, "default");
    if (IS_ERR(pins_default)) {
        ret = PTR_ERR(pins_default);
    }

    pins_eint_int = pinctrl_lookup_state(hotplug_pinctrl, "state_eint_as_int");
    if (IS_ERR(pins_eint_int)) {
        ret = PTR_ERR(pins_eint_int);
        return ret;
    }

    pinctrl_select_state(hotplug_pinctrl, pins_eint_int);
}
#endif//lisong[2015-12-11][BUGID:NULL][init sdcard hotplug pin state]end
/**
