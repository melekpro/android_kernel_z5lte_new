...
int switch_accdet_value = 1;
int ext_amp_on_flag = 0;
#endif
...
//lihua 2016-7-14[BUGID:NULL][ext-PA ctrl] start
extern unsigned int aud_ext_pa_mode;
...
static void AUDIO_EXT_PA_MODE0(void)
{
    AudDrv_GPIO_EXTAMP_Select(true);
    udelay(2);
}

static void AUDIO_EXT_PA_MODE1(void)
{
    AudDrv_GPIO_EXTAMP_Select(true);
    udelay(2);
...
static void AUDIO_EXT_PA_MODE2(void)
{
    AudDrv_GPIO_EXTAMP_Select(true);
    udelay(2);
    AudDrv_GPIO_EXTAMP_Select(false);
...
static void AUDIO_EXT_PA_MODE3(void)
{
    AudDrv_GPIO_EXTAMP_Select(true);
    udelay(2);
    AudDrv_GPIO_EXTAMP_Select(false);
...
//lihua 2016-7-14[BUGID:NULL][ext-PA ctrl] end


#define NULL_PIN_DEFINITION    (-1)
static void Ext_Speaker_Amp_Change(bool enable)
{
#define SPK_WARM_UP_TIME        (80)	/* unit is ms */
#ifndef CONFIG_FPGA_EARLY_PORTING
#if defined(CONFIG_MTK_LEGACY)
	int ret;
...
#else
        //lihua 2016-7-14[BUGID:NULL][ext-PA ctrl] start
        switch(aud_ext_pa_mode) {
            case AUDIO_PA_MODE0:
                AUDIO_EXT_PA_MODE0();//  1.2W
                break;
            case AUDIO_PA_MODE1:
                AUDIO_EXT_PA_MODE1();//  1.0W
                break;
            case AUDIO_PA_MODE2:
                AUDIO_EXT_PA_MODE2();//  0.8W
                break;
            case AUDIO_PA_MODE3:
                AUDIO_EXT_PA_MODE3();//  0.6W
                break;
            default:
                //
                break;
